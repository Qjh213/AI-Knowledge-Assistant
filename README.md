...
- 文档解析采用本地解析与 MinerU 双通道；失败文档可以明确选择解析器重试。
- 对超过 MinerU 官方 200 页限制的 PDF 在服务端透明拆分为每段 180 页，逐段解析后合并，用户侧仍保留一份原始文档。
- 本地解析和统一入库入口均执行文本质量检查，检测到乱码时终止处理并清理残留分块与向量，避免污染知识库。
- 检索采用 Milvus 稠密向量与原生 BM25 双路召回、RRF 融合、语义重排及失败降级。
- 评测集绑定源文档 SHA-256，支持语料预检、断点续跑及检索/端到端两种模式。

当前 Python 跨领域知识库实测包含 3 份 PDF，其中一份 402 页文档经自动拆分后成功生成 637 个分块；9 个检索问题的 `Hit@5 = 1.00`、`MRR = 0.8148`。该结果只代表当前固定数据集，不等同于通用准确率。
...
### MinerU 提示 PDF 超过 200 页

MinerU 官方接口限制单个文件最多 200 页。系统会在后台将超限 PDF 按每段 180 页透明拆分、依次解析并合并结果，无需用户手工拆文件。当前版本按顺序处理各段；如果进程在中途退出，会从第一段重新开始，尚未实现分段级断点续传。
